<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="RESULT" :formatter="tableFormatter" prop="RESULT" max-width="8%" align="center"/>
      ...
      >
        <div class="lb">
          <el-form-item label="STAGE" prop="STAGE" span="11">
            <el-input v-model="editFrom.STAGE"/>
          </el-form-item>
          ...
          <el-form-item label="Myan OPEN_TIME">
          ...
          <el-form-item label="Myan CLOSE_TIME">
            <div style="display: flex;flex-direction: column;">
              <el-date-picker
          ...
          <el-form-item label="BAN_NUMS" prop="BAN_NUMS" span="11">
            <el-input
              ...
              @input="validateInput"
            />
            <!--            oninput="value=value.replace(/,,/, ',').replace(/[^0-9,]|(,\d{4,})/g, '')"-->
            <!--            />-->
          </el-form-item>
      ...
      <el-collapse v-model="activeName" v-if="editFrom.ID">
        ...
        <el-collapse-item title="Detail" name="1">
          ...
          >
            <div class="lb">
              <el-row>
                <el-form-item label="Bet Total:">
                  <el-input v-model="bet_sum_total" readonly></el-input>
                </el-form-item>
...
<script>
  ...
  import {adjust_obj_prop, parseTime, remove_obj_prop} from "@/utils";
  ...
  import {
    ...
    settlement3DDigit,
    cancel3DDigit,
    reverse3DDigitSettle,
    get3DDigitDetail,
    get3DDigitHistoryDetail, getDigitDetail,
  } from "@/api/digit";
  ...
  export default {
    ...
    methods: {
      ...
      noonRadioChange(val) {
        ...
        this.editFrom.STAGE = this.editFrom.STAGE.replace('am', noon).replace('pm', noon).replace('AM', noon).replace('PM', noon)
      },
      radioChange() {
        if (this.radio === "2") {
      ...
      getList() {
        ...
        get3DDigitList(this.listQuery).then((response) => {
      ...
      getDigitDetail(match_id, evt) {
        ...
        (_this.editFrom.STATUS < 3
            ? get3DDigitDetail(para)
            : get3DDigitHistoryDetail(para)
        ).then((res) => {
          ...
          for (let i = 0; i < 1000; i++) {
            let temp = {
              ...
              money: '0',
              num_limit: _this.numFormat(limit_num)
            }
            res.items.forEach((ele) => {
              if (ele.BET_TYPE.padStart(3, '0') === temp.num) {
      ...
      handleSettlement() {
        ...
        settlement3DDigit(para).then((res) => {
          loading.close();
          if (res.code === 20000) {
            _this.$message({
      ...
      resetTemp() {
        ...
        let STAGE = date.getFullYear() + '' + (month < 10 ? "0" + month : month) + '' + (day < 10 ? "0" + day : day)
      ...
      createData() {
        ...
        para.OPEN_MD_TIME = dateToString(new Date(stringToDate(para.OPEN_TIME).valueOf() - 5400 * 1000))
        para.CLOSE_MD_TIME = dateToString(new Date(stringToDate(para.CLOSE_TIME).valueOf() - 5400 * 1000))
        if (para.BAN_NUMS) {
          para.BAN_NUMS = Array.from(new Set(para.BAN_NUMS.split(',')))
            .sort((a, b) => a - b)
            .join(',')
        }
        add3DDigit(para).then((response) => {
      ...
      refresh_date() {
        ...
        this.intervalID = setInterval(() => {
          let _this = this
          if (_this.$route.path !== '/match/3D-digital-list' || !_this.dialogFormVisible) {
      ...
      updateData() {
        ...
        edit3DDigit(para).then((response) => {
          if (response.code === 20000) {
            _this.$message({
              message: response.message,
              type: "success",
      ...
      deleteDigit: function () {
        ...
        del3DDigit({ID: this.selectedDigit.ID}).then((res) => {
          this.listLoading = false;
          if (res.code === 20000) {
      ...
      tableFormatter(row, column, cellValue, index) {
        ...
        switch (column.property) {
          case 'RESULT':
            str = this.listQuery.game_type == 1 ? this.padding_zero(String(row.RESULT), 2) : this.padding_zero(String(row.RESULT), 3)
            break
          case 'OPEN_TIME':
      ...
      reverse_cancel_match(match, type = 'cancel') {
        ...
        (type === 'cancel' ? cancel3DDigit(para) : reverse3DDigitSettle(para)).then((res) => {
          if (res.code === 20000) {
            _this.$message({
