      ...
      <el-row>
        <el-col :span="18">
          ...
          <el-input
            ...
            class="filter-item"
          />
          <el-select v-model="listQuery.game_type" @change="getList" placeholder="">
...
<!--          <el-button icon="el-icon-thirdzanting" @click="openConfirmDia(false)" type="danger">Stop all</el-button>-->
    ...
    <el-table
      ...
      <el-table-column label="RESULT" prop="RESULT" max-width="6%" :formatter="tableFormatter" align="center"/>
  ...
  export default {
    ...
    data() {
      return {
        ...
        permission: {},
        routeID: 59,
        status_dict:['Listing','Waiting', 'Pause', 'Settling', 'Finished', ],
        status_type_dict:['Settle', 'Cancel', 'Reverse', '', ],
        listQuery: {
          page: 1,
          limit: 99,
          start_time: null,
    ...
    methods: {
      ...
      tableFormatter(row, column, cellValue, index) {
        ...
        switch (column.property) {
          case 'STATUS':
            str =  this.status_dict[row.STATUS];
            break
          case 'SETTLE_TYPE':
            str =  this.status_type_dict[row.SETTLE_TYPE];
            break
          case 'RESULT':
            str =  row.STAGE.indexOf('M') > -1 ? this.padding_zero(String(row.RESULT),2):this.padding_zero(String(row.RESULT),3)
            break
        }
        return str
      },
