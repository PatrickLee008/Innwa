...
import { getOrderCopyList } from '@/api/order'
...
export default {
  ...
  methods: {
    getList() {
      ...
      if (!this.listQuery.end_time && !this.listQuery.start_time) {
      ...
      getOrderCopyList(this.listQuery).then((response) => {
    ...
    showDetails(order_id) {
      ...
      getOrderCopyList(para).then((res) => {
