...
from app_server.model.DigitalModel import Digital, DigitStatus
...
digital = Blueprint('digital', __name__)


# 获取订单列表
@digital.route('', methods=['GET'])
# @auth.login_required
def get_digit_list():
    """
                    @@@
                    ...
                    #### Returns::
                            {
                                'code': 20000,
                                'items': [u.to_dict() for u in digit_list],
                            }
                """
    ...
    try:
        ...
        digit_list = Digital.query

        if start_time:
            digit_list = digit_list.filter(db.cast(Digital.CREATE_TIME, db.Date) >= start_time)

        if end_time:
            digit_list = digit_list.filter(db.cast(Digital.CREATE_TIME, db.Date) <= end_time)

        if key_word:
            digit_list = digit_list.filter(Digital.STAGE.like('%{}%'.format(key_word)))

        if status:
            digit_list = digit_list.filter(Digital.STATUS == status)

        total = digit_list.count()

        digit_list = digit_list.offset((page - 1) * limit).limit(limit).all()
...
# 获取订单列表
@digital.route('/get_actvie_nper', methods=['GET'])
@auth.login_required
def get_active_stage():
    digit = Digital.query.filter(Digital.STATUS == DigitStatus.Opened,
                                 db.cast(Digital.OPEN_TIME, db.Date) == datetime.date.today()).order_by(
        Digital.STAGE.desc()).first()
    # 查询最近30期的开奖结果
    ...
    digital_list = Digital.query.filter(db.cast(Digital.OPEN_TIME, db.Date) >= seven_day_pass).order_by(
        Digital.CREATE_TIME.desc()).all()
    result = []
    if len(digital_list) > 0:
        for temp in digital_list:
            if temp.STATUS == 3:
                temp = temp.to_dict()
                result.append({
...
# 获取订单列表
@digital.route('/digit_bet', methods=['POST'])
@auth.login_required
def digit_bet():
    """
                @@@
    ...
    try:
        ...
        digit_stage = Digital.query.filter_by(ID=match_id).one_or_none()

        if not digit_stage:
            return jsonify({
                'code': 50002, 'message': "No match open"
            })
        ...
        if digit_stage.STATUS != DigitStatus.Opened:
            return jsonify({
...
@digital.route('/get_detail', methods=['GET'])
@auth.login_required
def get_digital_detail():
    ...
    for bet_type, bet_money in _sum:
        ...
        per_limit_code = limit_num
        if bet_money > digit_stage.LIMIT_CODE * limit_num:
            per_limit_code += digit_stage.EX_LIMIT
        sum_dict[bet_type] = {
            "BET_MONEY": str(bet_money),
            "LIMIT_NUM": per_limit_code
        }
    for u in order_list:
        temp = u.to_dict()
