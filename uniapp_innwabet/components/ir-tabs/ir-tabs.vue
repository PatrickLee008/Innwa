<template>
	<view :style="{width, overflow: 'hidden'}">
		<!-- nav -->
		<view v-if="!noNavi" class="ir-nav-con" :style="{width, background: navBgColor}">
		...
		<view :class="noNavi ?'':'ir-page-con'" :style="{transform: `translateY(${transY}rpx)`, height: pageH}">
			<!-- loagding -->
			...
			<view
				...
				<view class="ir-page" :style="[pageStyle]">
					<slot name="page9">9</slot>
				</view>
				<view class="ir-page" :style="[pageStyle]">
					<slot name="page10">10</slot>
				</view>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		computed: {
			// 页面除去导航栏的高度
			pageH() {
				return this.noNavi ?`calc(${this.height})`:`calc(${this.height} - 84rpx)`
			}
		},
		watch: {
			loading(val) {
