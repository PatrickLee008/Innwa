		...
		<form class="mybg-grey full-page">
			<view class="myrect box-shadow bg-white flex-column text-bold"
				...
				<view class="flex-row" style="width: 96vw;margin: 1px;justify-content: flex-start;flex-wrap: wrap;">
					<view class="myrect mybg-dgrey flex-column"
						style="width: 17%;height: 17vw;margin: 1px 0 5px calc(25vw / 10);line-height: 6vw;"
						v-for="(day,index) in results_of_day" v-if=" 0 < day.day && day.day < 6" :key="index">
						<text>{{day.label}}</text>
						<view class="flex-row" style="justify-content: space-around;height: 6vw;">
							<text class="mycolor-red">{{day.am}}</text>
							<text class="mycolor-orange">{{day.pm}}</text>
						</view>
					</view>
				</view>
			</view>
	...
	export default {
		...
		methods: {
				...
				items.forEach((ele, index) => {
					all_day.forEach((_ele, _index) => {
						if (_ele.date == ele.OPEN_TIME.slice(0, 10) && ele.STATUS == 3) {
							let apm = ele.STAGE.slice(-2)
							let res = ele.RESULT.toString()
							_ele[apm.toLocaleLowerCase()] = reg.test(res) ? '0' + res : res
						}
					})
				})
			...
			set_day(btn) {
				...
				btn.selected = true
				_this.listQuery.end_time = _this.getLastDay(dateFormatUtils.stringToDate2(date_btn.date))
