...
def settle(match_id, host_team_result, guest_team_result):
    try:
        ...
        # 更新订单比分结果
        orders = session.query(Order).filter(Order.MATCH_ID == match_id).all()
        for od in orders:
            od.BET_HOST_TEAM_RESULT = host_team_result
            od.BET_HOST_TEAM_RESULT = guest_team_result

        # 处理已结算混合订单的比分
        order_histories = session.query(OrderHistory).filter(OrderHistory.MATCH_ID == match_id).all()
        for oh in order_histories:
            oh.BET_HOST_TEAM_RESULT = host_team_result
            oh.BET_HOST_TEAM_RESULT = guest_team_result
        session.commit()

        # # 查询所有单笔订单
        single_orders = session.query(Order).filter(Order.MATCH_ID == match_id, Order.IS_MIX == "0", Order.STATUS != "0").all()
        print("got single orders:", single_orders)
        ...
        # # 1.查询出包含该比赛号,并且有效,混合注的订单
        blend_orders = session.query(Order).filter(and_(Order.MATCH_ID == match_id, Order.IS_MIX == "1", Order.STATUS != "0")).all()
        # 混合佣金比例
        blend_ratio = int(session.query(MDict).filter_by(MDICT_ID="666").one().CONTENT) * 0.01
        blend_order_ids = [(order.ID, blend_ratio) for order in blend_orders]
