...
from management_server.model.DigitalModel import Digital, DigitStatus
...
r_digital = Blueprint('digital', __name__)


@r_digital.route('/get', methods=['GET'])
@auth.login_required
def get_digital_list():
    """
                    @@@
                    #### Args:
                           {
                    ...
                    #### Returns::
                            {
                                'code': 20000,
                                'items': [u.to_dict() for u in digital_list],
                            }
                """
    ...
    digital_list = Digital.query.order_by(Digital.ID.desc())
    if key_word:
        digital_list = digital_list.filter(
            or_(Digital.ID.like('%{}%'.format(key_word)), Digital.TITLE.like('%{}%'.format(key_word)),
                Digital.CONTENT.like('%{}%'.format(key_word))))

    if start_time:
        digital_list = digital_list.filter(Digital.CREATE_TIME >= start_time)

    if end_time:
        end_time += ' 23:59:59'
        digital_list = digital_list.filter(Digital.CREATE_TIME <= end_time)

    total, total_bet_count, total_bet_sum, total_benefit = digital_list.with_entities(func.count(Digital.ID),
                                                                                      func.sum(Digital.BET_NUM),
                                                                                      func.sum(Digital.BET_SUM),
                                                                                      func.sum(Digital.BENEFIT)).one()
    print("-----", total, total_bet_count, total_bet_sum, total_benefit)

    digital_list = digital_list.offset((current_page - 1) * limit).limit(limit).all()
...
@r_digital.route('/get_detail', methods=['GET'])
@auth.login_required
def get_digital_detail():
    """
                @@@
                #### Args:
                        current_page = request.args.get('page', type=int, default=1)
    ...
    digit_stage = Digital.query.filter_by(ID=match_id).first()
...
@r_digital.route('/get_history_detail', methods=['GET'])
@auth.login_required
def get_digital_history_detail():
    """
                @@@
                #### Args:
                        current_page = request.args.get('page', type=int, default=1)
...
# 结算比赛
@r_digital.route('/settle', methods=['POST'])
# @auth.login_required
def settle_digit():
    """
        @@@
    ...
    try:
        digit = Digital.query.filter_by(ID=digit_id).first()

        if not digit:
            return jsonify({'code': 50002, 'message': "digit does not exist"})
        if digit.STATUS == DigitStatus.Settled:
            return jsonify({'code': 50002, 'message': "digit is game over."})
...
@r_digital.route('/edit', methods=['POST'])
@auth.login_required
def edit_digital():
    """
    @@@
    ...
    args = request.get_json()
    edit_id = args.get('ID')
    print("edit_digital", args)
    try:
        digital = Digital.query.filter_by(ID=edit_id).one_or_none()
        ...
        OrmUttil.set_field(digital, args)
        print("whose doing this shit?", g.user.ACCOUNT)
        digital.UPDATER = g.user.ACCOUNT
        db.session.commit()
        user_opt.send({
            "operate": "修改数字盘",
            "route": "数字盘管理",
...
@r_digital.route('/add', methods=['POST'])
@auth.login_required
def add_digital():
    """
    @@@
    ...
    try:
        old_digit = Digital.query.filter_by(STAGE=stage).first()
        if old_digit:
...
@r_digital.route('/remove', methods=['POST'])
@auth.login_required
def remove_digital():
    """
        @@@
    ...
    try:
        Digital.query.filter_by(ID=remove_id).delete()
        db.session.commit()
        user_opt.send({
            "operate": "删除数字盘",
            "route": "数字盘管理",
